...
// Compact card for services not in API data
function ClientServiceCard({ service }) {
  const Icon = iconMap[service.iconName] || FaHardHat;
...
export default function Service() {
  const [services, setServices] = useState({ active: [], client: [] });
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        // Use getServicesWithChefs for the public services page
        const data = await getServicesWithChefs();
        ...
        const client = Object.entries(serviceConfig)
