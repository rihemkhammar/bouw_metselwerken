...
const ProjectDetail = () => {
  ...
  const getStatusConfig = (status) => {
    ...
    return config[status] || { label: status, class: 'bg-gray-100 text-gray-800 border-gray-200' };
  };
  ...
  const getUpdateTypeConfig = (type) => {
    ...
    return config[type] || { label: type, class: 'bg-gray-100 text-gray-800 border-gray-200' };
  };
          ...
          <div className="flex items-center gap-4 justify-start">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-muted mt-1">
                ID: <code className="bg-off-white px-1.5 py-0.5 rounded text-xs border border-border">{project.id}</code>
              </p>
            </div>

            <span className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${statusConfig.class}`}>
              {statusConfig.label}
            </span>
          </div>
        </div>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-border p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-gradient-to-br from-success/10 to-success/5 rounded-xl p-4 border border-success/30">
              <p className="text-xs font-medium text-success uppercase tracking-wide">Budget</p>
              <p className="text-2xl font-bold text-success mt-1">{formatBudget(project.budget)}</p>
            </div>
            
            <div className="bg-off-white rounded-xl p-4 border border-border">
              <p className="text-xs font-medium text-muted uppercase tracking-wide">Service</p>
              <p className="text-lg font-semibold text-primary mt-1">{getServiceLabel(project.services)}</p>
            </div>
            
            <div className="bg-off-white rounded-xl p-4 border border-border">
              <p className="text-xs font-medium text-muted uppercase tracking-wide">Statut</p>
              <p className="text-lg font-semibold text-primary mt-1">{statusConfig.label}</p>
            </div>
            
            <div className="bg-off-white rounded-xl p-4 border border-border">
              <p className="text-xs font-medium text-muted uppercase tracking-wide">Client ID</p>
              <p className="text-xs font-mono text-secondary mt-1 truncate" title={project.clientId}>{project.clientId}</p>
            </div>
          </div>

          <div>
            <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-2">Description</label>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="flex items-start gap-4 p-4 bg-off-white rounded-xl border border-border">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-primary text-lg">{project.chef.name}</p>
                  <p className="text-sm text-muted">Matricule: <code className="bg-white px-1 rounded">{project.chef.matricule || 'N/A'}</code></p>
                </div>
              </div>

              <div className="p-4 bg-off-white rounded-xl border border-border space-y-3">
                <div className="mt-3 space-y-2">
                  <a href={`mailto:${project.chef.email}`} className="flex items-center gap-2 text-sm text-secondary hover:text-primary transition-colors">
                    <HiEnvelope className="w-4 h-4 text-primary" />
                    {project.chef.email}
                  </a>
                  <a href={`tel:${project.chef.phone}`} className="flex items-center gap-2 text-sm text-secondary hover:text-primary transition-colors">
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-border p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="flex items-start gap-4 p-4 bg-off-white rounded-xl border border-border">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-primary text-lg">{project.client.name}</p>
                  
                  <div className="mt-3 space-y-2">
                    <a href={`mailto:${project.client.email}`} className="flex items-center gap-2 text-sm text-secondary hover:text-primary transition-colors">
                      <HiEnvelope className="w-4 h-4 text-primary" />
                      {project.client.email}
                    </a>
                    <a href={`tel:${project.client.phone}`} className="flex items-center gap-2 text-sm text-secondary hover:text-primary transition-colors">
                      <HiPhone className="w-4 h-4 text-primary" />
              ...
              <div className="p-4 bg-off-white rounded-xl border border-border space-y-3">
                <div>
                  <p className="text-xs font-medium text-muted uppercase">Entreprise</p>
                  <p className="text-secondary">{project.client.companyName || 'Particulier'}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-muted uppercase">Adresse</p>
                  <p className="text-secondary">{project.client.address || 'Non renseignée'}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-muted uppercase">ID Client</p>
                  <p className="text-xs font-mono text-secondary truncate" title={project.client.id}>{project.client.id}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          {project.updates?.length > 0 ? (
            <ul className="space-y-4">
              {project.updates.map((update, index) => {
                ...
                return (
                  <li key={update.id || index} className="p-4 bg-off-white rounded-xl border border-border">
                    <div className="flex flex-wrap items-center justify-between gap-2 mb-3 pb-3 border-b border-border">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`px-2.5 py-1 text-xs font-semibold rounded-full border ${typeConfig.class}`}>
                            {typeConfig.label}
                          </span>
                    ...
                    <div className="space-y-3">
                      {(update.message || update.content || update.details) && (
                        <p className="text-secondary pl-1">{update.message || update.content || update.details}</p>
                      )}
                      ...
                      <div className="flex flex-wrap gap-3 pt-3 border-t border-border text-xs text-muted">
                        {update.updatedBy && (
                          <span className="flex items-center gap-1">
                            ...
                            <code className="bg-white px-1.5 py-0.5 rounded">{update.updatedBy.slice(0, 8)}...</code>
                          </span>
                        )}
                        {update.id && (
                          <span className="flex items-center gap-1">
                            <HiHashtag className="w-3.5 h-3.5 text-primary" />
                            <code className="bg-white px-1.5 py-0.5 rounded">{update.id.slice(0, 6)}...</code>
                          </span>
                        )}
                      </div>
                    </div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-secondary">Aucune mise à jour pour le moment.</p>
            </div>
          )}
        </section>
